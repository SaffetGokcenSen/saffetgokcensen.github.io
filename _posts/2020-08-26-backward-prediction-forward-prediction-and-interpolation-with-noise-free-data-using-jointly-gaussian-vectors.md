...
## Conclusion
A numerical implementation is on  [this link](https://github.com/SaffetGokcenSen/Gaussian-Models/blob/master/estimating_noise_free_data_using_jointly_gaussian_vectors.ipynb). All of the algorithm and the numerical results depend on the assumption that the posterior is proper although the prior is improper. Another assumption is that the inverses are assumed to exist although they cannot be formally defined. Hence, the numerical results must be examined with these assumptions in mind. In addition, the coefficient $\lambda$ does not have any effect on the MAP estimate value. However, $\lambda$ effects our uncertainty about the MAP estimate. The interpolation results are more successful than the prediction results.
